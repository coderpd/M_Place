...
const VendorPOAutomationPage = () => {
    ...
    const fetchPurchaseOrders = async () => {
      try {
        ...
        const response = await fetch(
          `http://localhost:5000/PoVendorUser/vendor/${vendorData.id}`
        );
  ...
  const handleDownloadPDF = async (poId) => {
    ...
    try {
      const response = await fetch(
        `http://localhost:5000/PoVendorUser/generate-pdf/${poId}`
      );
  ...
  const handleSaveChanges = async () => {
    try {
      const response = await fetch(
        `http://localhost:5000/PoVendorUser/update/${selectedPO.id}`,
        {
          method: "PUT",
          headers: {
      ...
      {selectedPO ? (
        <div className="bg-white rounded-lg shadow-md overflow-hidden border border-gray-200">
          ...
          <div className="p-6">
            ...
            {/* Edit buttons
            {editing && (
              <div className="mt-4 pt-4 border-t border-gray-300 flex justify-end gap-3">
                <Button
                  onClick={handleCancelEdit}
                  variant="outline"
